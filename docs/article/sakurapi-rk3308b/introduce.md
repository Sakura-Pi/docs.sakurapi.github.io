...
#### LLVMPipe GLSL
:::info
GitHub 仓库 **[astefanutti/kms-glsl](https://github.com/astefanutti/kms-glsl)**  

kms-glsl 可以调用 GPU 渲染 glsl 着色器，但由于 RK3308B 并没有 GPU  
这将导致 Linux 使用 LLVMPipe 强制进行软件渲染。
:::

我们使用 LLVMPipe 去渲染 glsl 着色器，以CPU 1.3GHz的频率进行压力测试。在未加任何散热情况下，结温在140度左右。
